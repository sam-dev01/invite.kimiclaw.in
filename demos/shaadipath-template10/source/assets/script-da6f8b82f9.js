  ...
  var ttkItems  = C.thingsToKnow || [];
    ...
    if (story.show === false) {
      ...
      if (storyMode === 'tags') {
        ...
        var stHashtag = story.customHashtag ||
        ...
        if (stLetterBody) {
          ...
          if (couple.bride && couple.groom) {
            ...
            sigTagEl.textContent = 'With love, ' + couple.bride + ' & ' + couple.groom;
        ...
        if (sigEl && couple.bride && couple.groom) {
          sigEl.textContent = 'With love, ' + couple.bride + ' & ' + couple.groom;
        }
      }
    }
    ...
    if (gallery.show === false || gallery.layout === 'skip') {
      ...
      var frameIds = ['glHero', 'glLeft', 'glRight', 'glBottom'];
      frameIds.forEach(function(fid, i) {
        if (!photos[i]) return;
        var frameEl = document.getElementById(fid);
        if (!frameEl) return;
        var img = frameEl.querySelector('img');
        if (img) { img.src = photos[i]; img.removeAttribute('onerror'); }
      });
    }
    ...
    if (rsvpBtn) {
      if (rsvp.mode === 'form' && rsvp.form_url) {
        ...
        var waMsg = encodeURIComponent('Hi ' + (couple.bride || '') + ' & ' + (couple.groom || '') + '! I would love to join your celebration!');
...
(function initNav() {
  ...
  document.addEventListener("click", e => {
    if (isOpen && !cluster.contains(e.target)) closeMenu();
  }, { passive: true });
  ...
  const navObs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      menuList.querySelectorAll(".nav-item").forEach(btn => {
